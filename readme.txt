== xPerson Lite ==
Author: SketchThemes, tikendramaitry
Author URI: https://sketchthemes.com/
License: GNU General Public License v3.0
License URI: https://www.gnu.org/licenses/gpl-3.0.html
Requires at least: 3.8
Tested up to: 4.7.3
Stable tag: 1.0.3

== Description ==
xPerson Lite is a just launched clean, creative Portfolio WordPress theme which builds a stunning resume and packs power in your personal portfolio site, fashion, photography, CV and music events. Now showcase your portfolio like you've never done before!

=========================
=		Theme Setup		=
=========================
1. First, install and activate the theme.
2. Create pages say
	Home - Add details about yourself or content of your choice..
	Works - Add your portfolio, services or works etc..
	Blog - For posts
3. Settings->Reading
	Set your site static front page and posts page to Home and Blog respectively.
4. xPerson Options->Static front page settings
	Select the timeline post category.
	Set the landing page.
5. To create smooth scroll links to timeline and landing page section configure the menu. Refer - https://sketchthemes.com/documentation/xperson/xperson-lite-menu-config.png

======================
=		Support		 =
======================
If you have any difficulties or issues with the theme then :-
1. Raise your support ticket on https://wordpress.org/support/theme/xperson-lite/ or Contact theme author on https://sketchthemes.com/.

======================
=		Credits		 =
======================

== PHP ==
* The TGM Plugin Activation library is licensed under the GPL-2.0 or later license. - https://tgmpluginactivation.com/

== CSS ==
* css/animate.css
Animate.css - https://daneden.me/animate
Copyright (c) 2014 Daniel Eden
Licensed under the MIT license - https://opensource.org/licenses/MIT

* css/bootstrap.css, css/bootstrap.min.css
Bootstrap v3.3.2 (https://getbootstrap.com)
Copyright 2011-2015 Twitter, Inc.
Licensed under MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)

* css/font-awesome.css, css/font-awesome.min.css
Font Awesome 4.7.0 by @davegandy - https://fontawesome.io - @fontawesome
License - https://fontawesome.io/license (Font: SIL OFL 1.1, CSS: MIT License)


== Images ==
* header.jpg
https://pixabay.com/en/girl-portrait-breton-brunette-1364004/
* breadcrumb.jpg
https://pixabay.com/en/wooden-fishing-pier-pier-recreation-1580779/
* All other images created by SkethThemes under GNU GPL.

== JS ==
* js/bootstrap.js, js/bootstrap.min.js
Bootstrap v3.3.2 (https://getbootstrap.com) | Copyright 2011-2015 Twitter, Inc. | Licensed under MIT
https://github.com/twbs/bootstrap/blob/master/LICENSE

* js/jquery.sticky.js
Sticky Plugin v1.0.0 for jQuery | Copyright 2014 Anthony Garand https://garand.me | Licensed under GPL3
https://github.com/garand/sticky

* js/smoothscroll.js
SmoothScroll v0.9.9 | Balazs Galambosi, Patrick Brunner, Michael Herf | Licensed under MIT

* js/wow.min.js, js/wow.js
WOW - v1.1.2 | Copyright (c) 2015 Matthieu Aussaguel | Licensed under MIT
https://github.com/matthieua/WOW

* js/html5shiv.js, js/html5shiv.min.js
HTML5 Shiv v3.7.0 | @afarkas @jdalton @jon_neal @rem | Licensed under MIT/GPL2
https://github.com/aFarkas/html5shiv

* js/respond.js, js/respond.min.js
Respond.js v1.4.2 | Scott Jehl. | Licensed under MIT Lic.
https://github.com/scottjehl/Respond


== Fonts ==
* Google Fonts Roboto
Roboto | Christian Robertson | Apache License, version 2.0
https://www.google.com/fonts/specimen/Roboto

* Google Fonts Roboto Slab
Roboto Slab | Christian Robertson | Apache License, version 2.0
https://www.google.com/fonts/specimen/Roboto+Slab

* Font Awesome 4.7.0
by @davegandy | Font: SIL OFL 1.1, CSS: MIT License
https://fontawesome.io/license/


== Changelog ==

= 1.0.4 (17 May 2017) =
* Added setup instructions.
* Added option to enable disable the timeline section.
* Updated setup instructions in readme.txt.

= 1.0.3 (10 Mar 2017) =
* Fixed review issues.

= 1.0.2 (02 Mar 2017) =
* Fixed review issues.

= 1.0.1 (10 Nov 2016) =
* Fixed review issues.

= 1.0.0 (25 MAY 2016) =
* Initial release